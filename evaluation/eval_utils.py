...
class InceptionV3(nn.Module):
    def __init__(self, model_path, num_classes):
        ...
        self.model.load_state_dict(torch.load(model_path, map_location='cuda:3'))
...
class CLIPScore:
    def __init__(self, device='cuda' if torch.cuda.is_available() else 'cpu'):
        self.model, _, _ = open_clip.create_model_and_transforms('ViT-H-14', pretrained='../DiFashion/models/huggingface/open_clip/open_clip_pytorch_model.bin')
...
def calculate_fid_given_data(gen4eval, grd4eval,
    ...
    # model_path = 'pretrained-models/pt_inception-2015-12-05-6726825d.pth'
...
def evaluate_personalization_given_data_sim(gen4eval, batch_size,
    ...
    with torch.no_grad():
        for gen, hist in gen_dataloader:
            gen = gen.to(device) # Tensor:(1,3,224,224)
            hist = hist.to(device) # Tensor:(1,1024)

            gen_emb = clip.model.encode_image(gen)
            gen_emb = gen_emb / gen_emb.norm(p=2, dim=-1, keepdim=True)

            hist = hist / hist.norm(p=2, dim=-1, keepdim=True) # Tensor:(1,1024)
            ...
            scores.append(sim_score[0].item())
...
def evaluate_minicpm_qua(minicpm_qua_dataset, batch_size, device, num_workers=1):
    ...
    minicpm_qua_dataloader = DataLoader(minicpm_qua_dataset,
                                batch_size=batch_size,
                                shuffle=False,
                                drop_last=False,
                                num_workers=num_workers)
    scores_qua = []
    with torch.no_grad():
        for image, prompt in minicpm_qua_dataloader:
            tensor_image = image.squeeze(0)
            numpy_image = tensor_image.numpy()
            image_qua = Image.fromarray(numpy_image)
            msgs_qua = [{'role': 'user', 'content': str(prompt)}]
            ...
            score_qua = extract_score(res_qua)
            scores_qua.append(score_qua)
    return scores_qua




def evaluate_personalization_for_each_gen(gen4eval_hist,gen4eval_image, batch_size,
                                            device, num_workers=1, similarity_func="cosine"):
    # evaluate the similarity between history and generated images
    clip = CLIPScore(device=device)

    gen = gen4eval_image.to(device) # Tensor:(3,224,224)
    hist = gen4eval_hist.to(device) # Tensor:(1024, )

    gen = gen.unsqueeze(0) # Tensor:(1,3,224,224)
    hist = hist.unsqueeze(0) # Tensor:(1,1024)
...
class CompatibilityEvaluator:
    def __init__(self, ckpt_path="", device='cuda' if torch.cuda.is_available() else 'cpu'):
        self.clip, _, _ = open_clip.create_model_and_transforms('ViT-H-14', pretrained='../DiFashion/models/huggingface/open_clip/open_clip_pytorch_model.bin')
        self.evaluator = FashionEvaluator(cnn_feat_dim=1024)
        # self.evaluator.load_state_dict(torch.load(ckpt_path))
        self.evaluator.load_state_dict(torch.load(ckpt_path, map_location='cuda:3'))
        self.device = device
    ...
    def evaluate_compatibility(self, outfits, cnn_feats, cnn_feats_gen):
        ...
        return scores
def evaluate_compatibility_VBPR(outfits, grd_outfits, gen4eval, cnn_feat_path, cnn_feat_gen_path,
                                      ckpt_path, batch_size, device, num_workers=1, model_path=None):
...
def evaluate_compatibility_given_data(outfits, grd_outfits, gen4eval, cnn_feat_path, cnn_feat_gen_path,
    ...
    return scores, grd_scores


# def evaluate_compatibility_for_each_img(outfit_compatibility, grd_outfit_compatibility, gen_imgs_compatibility, cnn_feat_path, cnn_feat_gen_path,
#     ckpt_path, batch_size, device, num_workers=1):
#     # grd_outfit_compatibility: iid_lists, generated_iid is negative and true_iid is positive
#     # gen_imgs_compatibility: generated images
#     model = CompatibilityEvaluator(ckpt_path, device)
#
#     if not os.path.exists(cnn_feat_path):
#         raise ValueError(f"Cnn features does not exist in {cnn_feat_path}!")
...
#         cnn_feats = np.load(cnn_feat_path, allow_pickle=True)  # [img_num, 1024]
#         cnn_feats = torch.tensor(cnn_feats).to(device)
#     cnn_feats_gen = model.extract_cnn_feats_gen(gen_imgs_compatibility, batch_size, num_workers)
#
#
#     outfits_loader = DataLoader(
#         outfits,
