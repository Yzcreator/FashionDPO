...
The checkpoint obtained after fine-tuning for five epochs using the FashionDPO framework has been [released](https://huggingface.co/AZhe1220/fashiondpo/tree/main). `checkpoint_ifashion` is fine-tuned on the iFashion dataset, while `checkpoint_polyvore` is fine-tuned on the Polyvore dataset.
