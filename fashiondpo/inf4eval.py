...
def parse_all_args():
    ...
    parser.add_argument(
        ...
        default="output/finetune_lora_2000/checkpoint-10000",
        required=False,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output/sample_ddim_eval_lora_result/ifashion_FITB_7_2000/checkpoint-10000",
...
def main():
    ...
    if args.data_processed:
        ...
        if args.mode == "test":
            test_fitb_dict = np.load(os.path.join(data_path, "processed", "sample_500.npy"), allow_pickle=True).item()
    ...
    if args.use_lora:
        ...
        # print("Initialized Lora Layer Weight: ")
        # for name, layer in lora_attn_procs.items():
    ...
    # print_conv_layer_diffusion = diffusion.unet.conv_in
    # print("Initialized diffusion Conv Weight: ", print_conv_layer_diffusion.weight)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            for i in range(len(models)):
                model = models.pop()
                if args.use_lora and isinstance(model, AttnProcsLayers):
                    tmp_unet = UNet2DConditionModel.from_pretrained(args.pretrained_model_untrained_parameters,
                                                                    subfolder="unet")
                    tmp_unet.load_attn_procs(os.path.join(input_dir))

                    print("---------------------Saved lora layer--------------------------------")
                    for proc_name, processor in tmp_unet.attn_processors.items():
                        print(f"Processor: {proc_name}")
                        for name, param in processor.named_parameters():
                            print(f"Value of {name}: {param.data}")
                        break
    ...
    # print_conv_layer_checkpoint = diffusion.unet.conv_in
    # print("Loaded diffusion Conv Weight: ", print_conv_layer_checkpoint.weight)

    # Prepare everything with our `accelerator`.
    logger.info("Prepare everything with our accelerator...")
    trainable_layers, optimizer, train_dataloader, ddim_scheduler = accelerator.prepare(
        trainable_layers, optimizer, train_dataloader, ddim_scheduler
    )
    ...
    for path in inf_list:
        ...
        accelerator.print(f"Resuming from checkpoint {args.pretrained_model_name_or_path}")
        # accelerator.load_state(os.path.join(args.output_dir, path))
        accelerator.load_state(args.pretrained_model_name_or_path)

        print("---------------------Loaded lora layer--------------------------------")
        for proc_name, processor in diffusion.unet.attn_processors.items():
            print(f"Processor: {proc_name}")
            for name, param in processor.named_parameters():
                print(f"Value of {name}: {param.data}")
            ...
            for scale in scale_list:
                ...
                with torch.autocast(
                    ...
                    for i,batch in tqdm(enumerate(test_dataloader), total=len(test_dataloader)):
                        ...
                        batch_outputs, _, _, _, _, _ = unwrapped_model.fashion_generation(
                            uids,
                            oids,
                            input_ids,
                            olists,
